...
class Ipswich_Events_Results_Data_Access {		
  ...
  // Gives details of the fields supported for each race, number of results etc
    ...
    GROUP BY r.id    
	  ORDER BY r.date DESC, mr.order ASC";
...
r.team as team
    ...
    ORDER BY name ASC";
  	...
  	public function get_race_winners($race_id) {
		$sql = "SELECT r.name, r.club, r.gender_position as genderPosition, IFNULL(r.gun_time, IFNULL(r.chip_time, r.result)) as time, p.sex
FROM `wp_ije_results` r
JOIN (
SELECT s.id as sexId, s.sex, rp.top_finishers as numberOfFinishers
...
ORDER BY sex, genderPosition";

		return $this->get_results($sql, 'get_race_winners');
	}
  
    	public function get_category_winners($race_id) {
		$sql = "SELECT r.name, r.club, r.category_position as categoryPosition, IFNULL(r.gun_time, IFNULL(r.chip_time, r.result)) as time, p.categoryCode, p.categoryDescription, p.sex
FROM `wp_ije_results` r
JOIN (
...
ORDER BY sex, categoryCode, categoryPosition";

		return $this->get_results($sql, 'get_category_winners');
	}
